...
def classify_main_part_by_area_and_color(
    img_bgr,
    contour,
    small_area_max=50_000,
    medium_area_max=120_000,
    color_area_fraction_thresh=0.5
):
    """
    Classify the main piece by its area and dominant interior color.
    ...
    small_area_max : int
        Maximum area for small pieces.
    medium_area_max : int
        Maximum area for medium pieces.
    color_area_fraction_thresh : float
        Minimum fraction of color pixels required.
    ...
    ------
    ValueError
        If detection fails or color-size mismatch.
    """
    area = cv2.contourArea(contour)
    ...
    dominant_pixels = counts[dominant_color]

    # Check sufficient color coverage
    fraction = dominant_pixels / float(area)
    if fraction < color_area_fraction_thresh:
        raise ValueError(
            f"Dominant color {dominant_color} has too few pixels "
            f"({fraction:.2f} < {color_area_fraction_thresh})"
        )

    # Verify color-size consistency
    if size_label == "BIG" and dominant_color != "blue":
        raise ValueError(f"Expected blue for BIG part, got {dominant_color}")
    if size_label == "MEDIUM" and dominant_color != "yellow":
        raise ValueError(f"Expected yellow for MEDIUM part, got {dominant_color}")
    if size_label == "SMALL" and dominant_color != "red":
        raise ValueError(f"Expected red for SMALL part, got {dominant_color}")

    return dominant_color, size_label, counts
